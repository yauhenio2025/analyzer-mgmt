#!/usr/bin/env python3
"""Migrate engine prompts to stage_context format in the database.
...
This script:
1. Reads engines from the database
...
Usage:
    python scripts/migrate_engines_to_stage_context.py [--dry-run] [--engine KEY]
...
def extract_analysis_type(engine: Engine) -> tuple[str, str]:
    """Extract the analysis type from engine metadata."""
    key = engine.engine_key or ""
    desc = engine.description or ""
    name = engine.engine_name or ""
...
def extract_id_field(engine: Engine) -> str:
    """Determine the ID field convention from the schema."""
    schema = engine.canonical_schema or {}
...
def build_stage_context(engine: Engine) -> dict[str, Any]:
    """Build stage_context from engine's existing prompts."""
    extraction_prompt = engine.extraction_prompt or ""
    curation_prompt = engine.curation_prompt or ""
    concretization_prompt = engine.concretization_prompt
    ...
    analysis_type, analysis_type_plural = extract_analysis_type(engine)

    # Get ID field
    id_field = extract_id_field(engine)

    # Get relationships
    key_relationships = extract_key_relationships(
        extraction_prompt,
        engine.canonical_schema or {},
    )
    ...
    return {
        ...
        "extraction": {
            ...
            "core_question": engine.researcher_question or f"What {analysis_type_plural} are present?",
...
async def migrate_engines(engine_key: Optional[str] = None, dry_run: bool = False):
    """Migrate engines in the database."""
    # Create engine
    ...
    async with async_session() as session:
        ...
        query = select(Engine)
        if engine_key:
            query = query.where(Engine.engine_key == engine_key)

        result = await session.execute(query)
        engines = result.scalars().all()

        print(f"{'DRY RUN: ' if dry_run else ''}Migrating {len(engines)} engines...")
        print()
        ...
        for eng in engines:
            try:
                ...
                if eng.stage_context:
                    print(f"Already migrated: {eng.engine_key}")
                    skip_count += 1
                    continue

                # Check if has old prompts
                if not eng.extraction_prompt:
                    print(f"Missing extraction_prompt: {eng.engine_key}")
                    error_count += 1
                    continue

                # Build stage_context
                stage_context = build_stage_context(eng)

                if dry_run:
                    framework = stage_context.get("framework_key", "none")
                    print(f"Would migrate: {eng.engine_key} (framework: {framework})")
                    success_count += 1
                    continue

                # Update engine
                eng.stage_context = stage_context
                await session.flush()

                framework = stage_context.get("framework_key", "none")
                print(f"Migrated: {eng.engine_key} (framework: {framework})")
                success_count += 1

            except Exception as e:
                print(f"Error migrating {eng.engine_key}: {e}")
                error_count += 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Migrate engine prompts to stage_context format"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    asyncio.run(migrate_engines(args.engine, args.dry_run))
